...
/**
 * @brief 用一次牛顿迭代快速计算平方根倒数。
 * @param x 正浮点输入。
 * @return 1/sqrt(x) 的近似值。
 * @note 使用联合体完成位表示转换，避免原指针强制转换在 GCC -O2 下违反严格别名规则。
 */
float invSqrt(float x) {
		float halfx = 0.5f * x;
		union {
		...
		converter.bits = 0x5f3759dfU - (converter.bits >> 1U);
		converter.value = converter.value *
		                  (1.5f - (halfx * converter.value * converter.value));
